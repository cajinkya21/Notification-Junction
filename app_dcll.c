...
void print_app(app_dcll * l) {
	...
	ptr = l->head;
	if(i != 0) {
		printf("\n> %s %d print_app() :App\t\t\tRegistered with\n\n", __FILE__, __LINE__);
		printf("\n==================================================\n");
	}
...
void print_np_key_val(app_node * temp) {
	...
	/* Traverse the trailing list of NPS the application registered with */	
	while(head) {
		...
		/* Traverse the key-val array of specific NP */
		while(vptr) {
...
/* Search a registration in the Application List, for the given appname and npname and 
 * return -1 if found, 
 * otherwise return 0 
 */
...
np_node *get_reg_list(app_dcll * l, char *appname, char *npname) {
	...
	if(ptr != NULL) {
		printf("> %s %d get_reg_list() : App %s found. Checking np %s in app's list\n",__FILE__, __LINE__, appname, npname);
		nptr = ptr->np_list_head;
